...
function Navbar() {
  ...
  return (
    <nav className='fixed top-0 flex justify-center w-full h-[60px] bg-white dark:bg-gray-200 dark:bg-gray-800 dark:text-white z-50'>
      <div className="flex justify-between items-center px-5 w-[1080px] h-full">
        ...
        <div className='flex gap-x-5'>
          {navigation.map((navigationElement, index)=>(
            <div className='hover:underline hover:cursor-pointer underline-offset-4' onClick={()=>setNavigation(index)} style={{textDecoration: (index==navPointer) ? "underline" : "none", textDecorationColor: (index==navPointer) ? "#3333FF": "white"}}>{navigationElement.text}</div>
          ))}
        </div>
      </div>
    </nav>
